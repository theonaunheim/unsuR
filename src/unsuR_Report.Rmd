...
Tool Developer: Corey Neskey | cneskey@protonmail.com | [github.com/cneskey](https://github.com/cneskey) | [linkedin.com/in/cneskey](https://linkedin.com/in/cneskey) | [twitter.com/cneskey](https://twitter.com/cneskey) | [cneskey.github.io/unsur](https://cneskey.github.io/unsuR/)
...
Risk Explorer {.storyboard}
=====================================

### Plan A {data-commentary-width=650}

```{r Viol_plotA}
PerRiskViol_plotA <- ggplot(data = df.m4A,
                            aes(x=fct_reorder(risk, loss, .fun = mean),
                                y=loss,
                                fill = fct_reorder(risk, loss, .fun = mean),
                                )) +
  geom_violin() +
  stat_summary(fun=mean, geom = "point", color="red")+
  labs(
    title="Simulated Loss Output - Plan A",
    subtitle = "Over a 12 month period",
    x = "Risk Scenario",
    y = "Loss") +
  scale_y_continuous(labels = dollar_format())+
...
ggplotly(PerRiskViol_plotA)
```

### Plan B {data-commentary-width=650}


```{r viol_plotB}
PerRiskViol_plotB <- ggplot(data = df.m4B,
                            aes(x=fct_reorder(risk, loss, .fun = mean),
                                y=loss,
                                fill = fct_reorder(risk, loss, .fun = mean),
                                )) +
  ...
  stat_summary(fun=mean, geom = "point", color="red")+
  labs(
    title="Simulated Loss Output - Plan B",
    subtitle = "Over a 12 month period",
    x = "Risk Scenario",
    y = "Loss") +
  scale_y_continuous(labels = dollar_format())+
  ...
  theme_classic()+
  theme(legend.position = 'none')
ggplotly(PerRiskViol_plotB)
```

### Plan C {data-commentary-width=650}

```{r viol_plotC}
PerRiskViol_plotC <- ggplot(data = df.m4C,
                            aes(x=fct_reorder(risk, loss, .fun = mean),
                                y=loss,
                                fill = fct_reorder(risk, loss, .fun = mean),
                                )) +
  ...
  stat_summary(fun=mean, geom = "point", color="red")+
  labs(
    title="Simulated Loss Output - Plan C",
    subtitle = "Over a 12 month period",
    x = "Risk Scenario",
    y = "Loss") +
  scale_y_continuous(labels = dollar_format())+
...
[Frequency formats](https://sites.google.com/view/coreyneskey/improving-cybersecurity-decision-making-by-reducing-expert-bias#h.p_WA7BG-Cgc50Q) communicate information to experts in a form that more closely resembles the natural sampling observed in animal foraging and neural networks. What is 1% in standard format would be "1 in 100" in frequency format.
